...
void AmulKernelLauncher(const float* input, const float* weight, float* output, int axis,
...
class AmulOp : public OpKernel {
 public:
  ...
  void Compute(OpKernelContext* context) override {
    ...
    if(axis==0){
        output_tensor_shape.AddDim(input_xSize);
    	...
    	output_tensor_shape.AddDim(input_ySize);
        output_tensor_shape.AddDim(3 * (input_zSize - 2));
    }  
    else if(axis==2){
        output_tensor_shape.AddDim(3 * (input_xSize - 2));  	
        output_tensor_shape.AddDim(3 * (input_ySize - 2));
        output_tensor_shape.AddDim(input_zSize);
    }                     
    ...
    AmulKernelLauncher(input_flat.data(), weight.data(), output_flat.data(), axis, batchSize, input_xSize, input_ySize, input_zSize, numChannels);
